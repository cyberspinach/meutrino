...
class OutgoingIncomingTest extends AbstractHglibTest {
  ...
  def testBookmarks {
    ...
    assertEquals(List(("bm1", client.tip().node.short)), other.incomingBookmarks())
    assertEquals(List(("bm1", client.tip().node.short)), client.outgoingBookmarks(path = Some("other")))
  }
